...
impl<T: ?Sized> MetaTable<T> {
        ...
        T: CastFrom<R> + 'static,
    {
        ...
        let fat = unsafe { FatPtr::from_ptr(casted_ptr) };
...
impl FatPtr {
    unsafe fn from_ptr<T: ?Sized>(t: &T) -> Self {
        use std::ptr::read;
    ...
    unsafe fn create_ptr<T: ?Sized>(&self, ptr: *const ()) -> *const T {
    ...
    T: ?Sized + 'a,
{
    ...
    fn next(&mut self) -> Option<<Self as Iterator>::Item> {
        ...
        let res_id: ResourceId = match self.tys.get(index) {
            Some(&x) => x.into(),
            None => return None,
        };
...
mod tests {
    ...
    fn get() {
        ...
        assert_eq!(
            table
                .get(&*world.resource::<ImplementorC>())
                .unwrap()
                .method1(),
            33
        );
        assert_eq!(
            table
                .get(&*world.resource::<ImplementorD>())
                .unwrap()
                .method1(),
            42
        );
